...
Instead, it defines a *verification semantics*: how programs are translated into logical formulas and checked by an SMT solver.
...
Each `func` generates a set of **verification conditions**:

* preconditions from `requires`
* postconditions from `ensures`
* implicit well-formedness conditions (bounds, type constraints)

These conditions are combined into a single logical formula of the form:

```text
requires ⇒ ensures
```

The function is accepted if and only if the formula is valid.
...
Katon semantics are intentionally simple:

* state is immutable and versioned
* mutation is logical rewriting
* time is explicit via SSA
* correctness is logical validity

There is no execution model to reason about.
